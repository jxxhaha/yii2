...
class AjaxUpload {
    ...
    public function __construct($form_name, $file_size,$upload_dir = "./Uploads") {
    ...
    public function upload($return_arr=false) {
     ...
     /*if (!is_dir($this->upload_dir2)) mkdir($this->upload_dir2, 0777); //上传目录不存在则创建
        ...
        $filename = $this->upload_dir2 . '/' . $fname;
        file_put_contents('upload1.txt',$file['tmp_name']);
        if (!move_uploaded_file($file['tmp_name'], $filename)) { //执行上传
            exit('upload error!'); //上传失败，错误未知
        } else {
            $arr = array('ok'=>1, 'filename'=> $this->upload_dir2 .'/'. $fname, 'size'=>$file['size']);
            if ($return_arr) {}
            else {
        ...
        if (!is_dir($this->upload_dir)) mkdir($this->upload_dir, 0777,true); //上传目录不存在则创建
        $file = $_FILES[$this->form_name];
        if ($file['error']==1 || $file['size']>($this->file_size*1024)) exit('1'); //上传失败，图片不能大于 $this->file_size k！
        switch ($file['error']) {
            case 3:
        ...
        //file_put_contents('upload1.txt',  move_uploaded_file($file['tmp_name'], $filename));
        if (!move_uploaded_file($file['tmp_name'], $filename)) { //执行上传
            exit('upload error!'); //上传失败，错误未知
           
        } else {
        
            $arr = array('ok'=>1, 'filename'=> $this->upload_dir .'/'. $fname, 'size'=>$file['size']);
            
            if ($return_arr) return $arr;
            else {
                echo stripslashes(json_encode(array('ok'=>$arr['ok'], 'filename'=>$arr['filename']))); exit;
            }
        }
    }
